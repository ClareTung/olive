...
* 查询优化的工作实际上是遵循一些原则让MySQL的优化器能够按照预想的合理方式运行。

### 向MySQL发送一个请求的时候，MySQL到底做了些什么？

![image-20210829143530772](https://cdn.jsdelivr.net/gh/ClareTung/ImageHostingService/img/image-20210829143530772.png)
...
* 在实际开发中，尽量**保持查询简单**且**只返回必需**的数据，**减小通信间数据包的大小和数量**是一个非常好的习惯，这也是查询中尽量避免使用`SELECT *`以及加上LIMIT限制的原因之一。
